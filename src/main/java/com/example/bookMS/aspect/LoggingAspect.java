package com.example.bookMS.aspect;
...
public class LoggingAspect {
    ...
    // Controller 메서드 실행 전
    @Before("execution(* com.example.bookMS.controller.*.*(..))")
    ...
    @AfterReturning(
            pointcut = "execution(* com.example.bookMS.controller.*.*(..))",
            returning = "result"
    )
    ...
    @Around("execution(* com.example.bookMS.service.*.*(..))")
    ...
    @AfterThrowing(
            pointcut = "execution(* com.example.bookMS.controller.*.*(..)) || " +
                    "execution(* com.example.bookMS.service.*.*(..))",
            throwing = "exception"
    )
    ...
    @Before("execution(* com.example.bookMS.repository.*.*(..))")
